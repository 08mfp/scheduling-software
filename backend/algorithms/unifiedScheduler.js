...
/** getIdString => unify docId or ObjectId to string */
...
/** generate all 15 pairwise matchups among 6 teams, storing competitiveness */
...
/** compute distances for all pairs of teams once */
...
/** generate all boolean patterns of length totalSlots with exactly matchWeeks "true" */
...
/** flatten rounds => single list of 15 fixtures */
...
/** unify fixture references to the same team objects */
...
/** checks if a team has 3 consecutive away matches factoring rest weeks */
...
/** Feasibility check of final fixtures:
 *  - each of 5 rounds must have 3 fixtures with no overlap
 *  - partial locks + last-year flips
 *  - no 3 consecutive away
 *  - home distribution = 2 or 3
 */
function isFeasible(fixtures, teams, partialLocks = {}, lastYearMap = {}) {
  ...
  for (let fx of fixtures) {
    ...
    if (partialLocks[idB] && typeof partialLocks[idB][roundNum] === "number") {
      // if B is locked as 1 => B is home => homeAssignment=0 => conflict
...
/** short gap penalty => if a gap < minGapDays => penalty = (minGapDays - gapDays) */
function computeShortGapPenalty(fixtures, teams, minGapDays) {
  ...
  fixtures.forEach((fx) => {
    const homeId =
      fx.homeAssignment === 1 ? getIdString(fx.teamA._id) : getIdString(fx.teamB._id);
    const awayId =
      fx.homeAssignment === 1 ? getIdString(fx.teamB._id) : getIdString(fx.teamA._id);
...
/** main cost breakdown => summation of all cost components. */
function computeCostBreakdown(fixtures, teams, distances, weights = {}) {
  ...
  for (let fx of fixtures) {
    ...
    // broadcast penalty => Fri 20:00 => count
...
/** summarized cost => just totalCost */
...
/**
 * Round assignment with backtracking + memoization + aggressive pruning.
 * We also allow multiple permutations in the calling function
 * (desc, asc, random for the order of matchups).
 */
function backtrackingAssignRoundsWithPattern(teams, matchups, pattern, orderStrategy = "desc") {
  let sorted = [];
...
/** Assign home/away => random init + last-year flips => local flipping => feasible check */
...
/** schedule date/time:
 * Rounds 1..4 => [Fri20, Sat14, Sat20, Sun14]
 * Round5 => super saturday => [Sat14, Sat16, Sat18]
 */
function scheduleDates(fixtures, pattern, season, teams) {
  ...
  // set of rivalries for prime time
...
/** create neighbor for local search => flip home/away, swap two fixtures' rounds, or reassign entire round */
function makeNeighbor(schedule, teams, distances, weights, partialLocks, lastYearMap) {
  ...
  if (moveType === "flipHomeAway") {
    ...
    for (let i = 0; i < roundFixtures.length; i++) {
      ...
      roundFixtures[i].weekSlotIndex = newR; // simplistic approach
    }
  }
...
/** run local search => adaptive temperature sim annealing + bigger jump moves */
 ...
 * - Per-team fixture breakdown
 * - Team Rankings + interest
 * - Round start dates
 * - Enhanced cost breakdown with one-liners
...
function buildFinalSummary(schedule, teams) {
  ...
  summary.push(`  - Travel StdDev (fairness): ${travelStdDev.toFixed(2)} (Penalizes large disparities)`);
  summary.push(`  - Competitiveness Penalty:  ${compPenalty.toFixed(2)} (High-interest matches earlier => bigger penalty)`);
  summary.push(`  - Broadcast Penalty:        ${broadcastPenalty.toFixed(2)} (Extra cost if more Friday-night matches than limit)`);
  summary.push(`  - Timeslot Penalty:         ${timeslotPenalty.toFixed(2)} (Non-prime or sub-optimal timeslots => penalty)`);
  summary.push(`  - Short-Gap Penalty:        ${shortGapPenalty.toFixed(2)} (Matches <6 days apart => penalty)`);
  summary.push(`  - Top2 Missed Slot Penalty: ${top2missedSlotPenalty.toFixed(2)} (#1 vs #2 not in R5 final slot => +15)`);
...
/**
 * Main scheduling function with improvements:
 *  - We generate rest patterns
 *  - For each pattern, try multiple permutations (desc/asc/random)
 *  - Round assignment with backtracking + pruning + memo
 *  - Home/away assignment + local flipping
 *  - Date/time assignment
 *  - Optional local search with bigger jump moves
 *  - Keep best cost
 *  - Build final summary with full details
 */
async function generateComprehensiveFairFixtures(
  ...
  // multiple permutations for matchups => desc, asc, random
  ...
  for (let pIndex = 0; pIndex < patterns.length; pIndex++) {
    ...
    for (let strategy of orderStrategies) {
      ...
      const roundRes = backtrackingAssignRoundsWithPattern(
        teams, matchups, pattern, strategy
      );
  ...
  // format final fixtures to output
...
/**
 * Optional multi-run approach: run the entire scheduling process X times,
 * keep the overall best from across all attempts.
 */
